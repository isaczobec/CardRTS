...
{
    float4 baseColor = SAMPLE_TEXTURE2D_ARRAY(_TileTextures, sampler_TileTextures, detailUV, layer);
    if (_TileBreakupStrength <= 0.0) return baseColor;

    float4 colorB = SAMPLE_TEXTURE2D_ARRAY(_TileTextures, sampler_TileTextures, RotateUV(detailUV, 1.3) * 1.71, layer);
    float4 colorC = SAMPLE_TEXTURE2D_ARRAY(_TileTextures, sampler_TileTextures, RotateUV(detailUV, -2.1) * 1.37, layer);
