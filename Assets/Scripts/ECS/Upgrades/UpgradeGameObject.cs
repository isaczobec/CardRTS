...
// grid (see UpgradeShopUIManager, where _priceText is wired) and, without a click/hover
// listener attached, as the small badge CardGameObject spawns per equipped upgrade (see
// CardGameObject.SetUpgradeIcons, where a badge prefab variant can leave _priceText/
// _unaffordableOverlay unwired since neither applies there). Deliberately minimal —
// everything else about an upgrade (title, description) lives on UpgradePreviewGameObject,
// the shop's side panel shown on hover.
public class UpgradeGameObject : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void BuildUpgrade(Sprite image, int shopGoldCost)
    {
        if (_image != null)
            _image.sprite = image;
