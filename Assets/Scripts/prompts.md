...
Can you to the audiomanager sound registry add a list of extra audio clips that are played overlayed the default sound with some probability? If sounds with probabilities adding to 1 exist, then one overlay sound should guaranteedly be played.

Can you please create a new Fire man card? Please base it of the ice man card. Create new renderable types for the troop and a projectile it fires. Please also create a reusable damage over time modifier component & system. Please create a new "Scorched" modifier ID and renderable modifier type. For this effect, i also want you to create another component called something like "StackingBurnDebuffComponent", and it should count the number of consequtive hits from the fire mans projectiles a target has endured, and then increase the damage of the damage over time debuff with 0.2x the attack damage stat of the fire man (up to an adjustable limit of 5 stacks). The damage over time modifier should have a field for period, make it equal to one seconds in ticks for this debuff. Make the debuff last for 10 seconds, and have its duration refreshed every time the fire mans projectile hits. Begin the burn at 2 damage/proc and make each stack give +2 damage per proc. Use the same proc system the ice mans projectiles does. The scorch debuff should be applied to all troops within a radius of some multiple of the troops range stat (use 0.3x). For now, the fire man should not have any abilities.

Can you please create three new cards - One called "Sawmill", one called "Quarry", and one called "Mine"? They should generate wood, stone, and metal periodically respectively. They should be created similarily to the currently existing basic building. Please make it generate 30 of the resource per minute when close to the clients own base, and make it generate 100 of the resource per minute when placed in the middle of the world (scaling linearly in between). Please make it cost 80 each of the other two reasources than the one it generates.